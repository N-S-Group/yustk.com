...
class DMTcaptcha{
 ...
 require(dirname(__FILE__).'/DMT-captcha-config.php');
			...
			$im=imagecreatefromgif(dirname(__FILE__)."/DMT_captcha_fonts/back.gif");
			$width = imagesx($im);
			$height = imagesy($im);
			$rc=mt_rand(120,140);
			...
			$px =$margin_left;
   			For($i=0;$i<$length;$i++){
									imagettftext($im,$font_size,0,$px, $margin_top,$font_color,dirname(__FILE__)."/DMT_captcha_fonts/CARTOON8.TTF",$this->keystring[$i]);
