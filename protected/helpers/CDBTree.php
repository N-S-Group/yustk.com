...
// $ID : an ID of the element
    ...
    function getNodeInfo($ID) {
        ...
        if(($query=$this->db->query($this->sql)) && ($this->db->num_rows($query) == 1) && ($Data = $this->db->fetch_array($query)))
            return array((int)$Data[$this->left], (int)$Data[$this->right], (int)$Data[$this->level],$Data['title']);
        else trigger_error("phpDbTree error: ".$this->sql.mysql_error($this->db->link), E_USER_ERROR);
    }
...
// Returns : true on success, or false on error
    function insert($ID, $data) {
        ...
        // inserting new record
        $this->sql = 'INSERT INTO '.$this->table.'('.$fld_names.') VALUES('.$fld_values.')';
        if(!($this->db->query($this->sql))) trigger_error("phpDbTree error: ".$this->db->error().$this->sql, E_USER_ERROR);
...
// Returns : a result id for using with other DB functions
    ...
    function enumChildren($ID, $start_level=1, $end_level=1, $field="cid") {
        ...
        $this->sql = $this->sqlComposeSelect(array(
            ...
            '_'.$this->table.'.'.$this->id.'=\''.$ID.'\''
                ...
                .$whereSql,"",""," categories.idNewspaper "
        ));

        $this->sqlOrderBy("categories.idNewspaper");
