<?$data = Clients::getData();?>
<div class="login">
    <span><b><?=$data['user']?></b></span> / <a href="<?=$this->createUrl("/login/exit")?>"><b>выход</b></a><br>
    <?if(count($data['clientAgreements'])){?>
        Выберите номер договора:
        <select id="clientAgreements">
            <?foreach($data['clientAgreements'] as $key):?>
                <option type="text" value="<?=$key['id'];?>"><?=$key['name'];?></option>
            <?endforeach;?>
        </select><br>
        <b>Баланс:</b> -590 руб. &nbsp;&nbsp;&nbsp;
        <?if ($this->section != "docs") {?><a href="<?=$this->createUrl("/cabinet/index")?>">Документы</a><?} else echo "<b>Документы</b>"?>&nbsp;&nbsp;&nbsp;
        <?if ($this->section != "invoices") {?><a href="<?=$this->createUrl("/cabinet/invoices")?>">Счета</a><?} else echo "<b>Счета</b>"?>&nbsp;&nbsp;&nbsp;
        <?if ($this->section != "requests" AND $this->section != "requests2") {?><a href="<?=$this->createUrl("/cabinet/requests")?>">Заявки</a><?} else echo "<b>Заявки</b>"?>&nbsp;&nbsp;&nbsp;
    <?}else{?>
        За данным клиентом не закреплен ни один договор<br>

    <?}?>
        <?if ($this->section != "contracts") {?><a href="<?=$this->createUrl("/cabinet/contracts")?>">Типовые договора</a><?} else echo "<b>Типовые договора</b>"?>&nbsp;&nbsp;&nbsp;
</div>

<script>
    var url = "<?=$this->path?>";
</script>