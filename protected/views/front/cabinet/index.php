<?if(Yii::app()->user->hasFlash('success') && Yii::app()->user->getFlash('success') == 'ok'){?>
<div class="remodal" data-remodal-id="modal_success">
    <h1>Вы успешно восстановили пароль</h1>
</div>
    <script>
        $(document).ready(function(){
            var inst = $.remodal.lookup[$('[data-remodal-id=modal_success]').data('remodal')];
            inst.open();
        });
    </script>
<?}?>


<div class="content main" style="border-top: 2px solid #d6d6d6">

    <h3>Документы</h3><br>
    <b class="blues">Ожидаемые документы</b>

    <table class="price">
        <thead>
        <tr>
            <td>№ документа<td>
            <td>Дата<td>
            <td>Наименование</td>
            <td>Скачать</td>
        </tr>
        </thead>
        <tbody>
        <tr>
            <td>Акт №17-55<td>
            <td>12.01.2015<td>
            <td>Акт сверки от 12.01.2015</td>
            <td><a href="">Cкачать</a></td>

        </tr>

        <tr>
            <td>Акт №19/55<td>
            <td>12.01.2015<td>
            <td>Акт сверки от 12.01.2015</td>
            <td><a href="">Cкачать</a></td>

        </tr>




        </tbody>

    </table>

<br><br>
    <b class="blues">Не предоставленные документы</b><br>
    Ниже представлен список документов, оригиналы которых вы не предоставили в нашу компанию:
    <table class="price">
        <thead>
        <tr>
            <td>№ документа<td>
            <td>Дата<td>
            <td>Наименование</td>

        </tr>
        </thead>
        <tbody>
        <tr>
            <td>Акт №17-55<td>
            <td>12.01.2015<td>
            <td>Акт сверки от 12.01.2015</td>


        </tr>

        <tr>
            <td>Акт №19/55<td>
            <td>12.01.2015<td>
            <td>Акт сверки от 12.01.2015</td>


        </tr>
        </tbody>

    </table>

</div>